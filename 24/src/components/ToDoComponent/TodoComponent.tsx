...
export const ToDoComponent = () => {
    ...
    const setCompletedElement = (id: number) => {
        ...
        const mappedLi: Todo[] = todosList.map(el => el.id === +id 
    ...
    const renderListItems = () => {
        return todosList.map(el => {
            ...
            return <ListItemComponent {...funObj} />
        })
    }
    ...
    const renderInput = () => showInput && <input
        type="text"
    ...
    return <>
        <h1>
            To-Do List
        <i className="fa fa-toggle-on" id="kopce" aria-hidden="true" onClick={onHideInput}></i>
        </h1>
            {}
            {renderInput()}
        <ul>
